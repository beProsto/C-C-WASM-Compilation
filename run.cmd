...
powershell clang --target=wasm32 -nostdlib '-Wl,--no-entry' '-Wl,--export-all' -o public/llvm/simple.wasm src/llvm/simple.c
echo Finished!
echo ---
...
powershell clang++ --target=wasm32 -std=c++17 -nostdlib '-Wl,--no-entry' '-Wl,--export-all' '-Wl,--allow-undefined' -o public/llvm/main.wasm src/llvm/main.cpp src/llvm/utils/src/malloc.c src/llvm/utils/src/winreqanim.cpp
echo Finished!
echo ---

echo Compiling the WAT code:
:: uses wat2wasm tool from wabt toolkit
powershell wat2wasm src/wat/testin_it.wat -o public/wat/testin_it.wasm
echo Finished!
echo ---
